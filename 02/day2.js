...
const rf = fs.readFile(filePath, "utf-8", (err, data) => {
  if (err) throw err;
  const content = data;
  processFile(content);
});
  ...
  lines.forEach((line) => {
    ...
    gameData.forEach((pull) => {
      if (pull.includes("red")) {
        ...
        const redCount = parseInt(
          pull.substring(startIndex, pull.indexOf("red")).trim(),
        );
        if (redCount > red) {
          red = redCount;
        }
      ...
      if (pull.includes("green")) {
        ...
        const greenCount = parseInt(
          pull.substring(startIndex, pull.indexOf("green")).trim(),
        );
        if (greenCount > green) {
          green = greenCount;
        }
      ...
      if (pull.includes("blue")) {
        ...
        const blueCount = parseInt(
          pull.substring(startIndex, pull.indexOf("blue")).trim(),
        );
        if (blueCount > blue) {
          blue = blueCount;
        }
