...
const dijkstra = (digits) => {
  ...
  while (!finished) {
    ...
    visited[r][c] = true;
    let minCoordinate = { row: 0, column: 0 };
  ...
  const path = [];
  let u = { row: rows - 1, column: columns - 1 };
  while (u) {
    path.unshift(u);
...
export const getPartOneSolution = (input) => {
  ...
  const digits = lines.map((line) => [...line].map((d) => parseInt(d, 10)));
  ...
  const grid = Array.from({ length: digits.length }, (_, r) =>
