...
const getHandType = (cards) => {
  ...
  const jokerCount =
    cardRankMap.get("J") === 1 ? cardCountsMap.get("J") ?? 0 : 0;
  ...
  if (cardCountsMap.size === 2) {
    if (counts.includes(4)) {
      if (jokerCount > 0) {
        // 4 Jokers or 1 Joker both make a 5 of a kind.
        return "5-of-a-kind";
      }
      return "4-of-a-kind";
    } else {
...
const myResult1 = fs.readFile("input.txt", "utf8", (err, data) => {
  if (err) throw err;
  console.log(`Result for Part1: ${getPartOneSolution(data)}`);
});
